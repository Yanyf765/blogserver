...
public class ArticleService {
    ...
    public Result<Map> getPageArticleByState(Integer state, Integer page, String keywords) {
        Result<Map> result = new Result<>();
        result.setCode("200");
        Map map = new HashMap<>();
        PageHelper.startPage(page, 10);
        Long uid = Util.getCurrentUser().getId();
        List<Article> articleByStateAll = articleMapper.getArticleByStateAll(state, uid, keywords);
        PageInfo<Article> pageInfo = new PageInfo<>(articleByStateAll);
        map.put("list",articleByStateAll);
